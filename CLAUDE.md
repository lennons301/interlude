...
- UI settings that are not model tiers (issue #171): the quota threshold is the first, so `SettingFieldView`'s model-specific half became a discriminated `detail` (`model-tier` | `percent`) and each panel narrows to the language it speaks — the endpoint hands back every settable field and a panel selects its own by what the value *means*, not by naming keys. Its control is a fixed set of chips rather than a free number: the spread offered is finer than the decision it feeds, a value outside it is refused *with the list* rather than clamped, and the row keeps the one-press `environment` way back. The threshold resolves through three layers where a tier has two — a tier may resolve to "let the CLI decide", a gate needs a number — so `resolveQuotaThreshold` ends on the built-in default rather than on nothing
