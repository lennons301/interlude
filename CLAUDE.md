...
- Who paid (issue #174): `runs.lane`/`runs.laneBilling` and `tasks.lane`/`tasks.laneBilling` record the execution lane a pass ran on and its billing kind, written at pass start (a follow-up turn re-records, latest wins — the direction that fails safe when a session's lane is switched mid-flight). The billing kind is stored rather than looked up, because `lanes.yaml` changes under a deployment and a retired lane would otherwise rewrite the billing history of every past run. `settings.meteredSpendConfirmedAt` is the confirm-once gate's one durable fact
