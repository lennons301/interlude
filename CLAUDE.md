...
- `runs.status = rate_limited` + `runs.resumeAfter` is the quota pause (issue #168): non-terminal, set at pass completion when the account refused the pass, cleared by nothing yet — resuming is #169. It is the only status that waits on a clock rather than on a human or a turn. A pass refused on a *tier-scoped* window never reaches it (issue #170) — it degrades instead
