...
Schema at `src/db/schema.ts`. Seven tables: `projects`, `tasks`, `messages`, `runs`, `settings`, `quota_state`, `metered_spend`.
...
- `tasks.kind` distinguishes interactive (default) / implement / review / triage; `tasks.runId` links a task to its run; `tasks.resumedFromTaskId` links a resumed pass to the one it continues (issue #169) — the lineage the attempt's *budget* follows, so a resume spends what its allowance has left rather than the whole allowance again; `tasks.triageResult` holds a finished triage pass's parsed exit until the sweep applies it (triage owns no run, so its spend is counted into the daily cap by kind in `spend.ts`)
