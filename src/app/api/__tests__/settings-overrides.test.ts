...
describe("execution lanes on /api/settings/overrides", () => {
  ...
  it("reports each harness the file names with its image and whether it is built (issue #219)", async () => {
    ...
    // and the two Codex lanes (issue #221) another, so the daemon is asked
    // once per harness — twice — in the order the file first names them.
    ...
    expect(state.harnesses).toEqual([
      ...
      { id: "codex", image: "interlude-agent-codex:latest", built: true },
    ]);
    expect(imageProbe.asked).toEqual([
      "interlude-agent-claude-code:latest",
      "interlude-agent-codex:latest",
    ]);
    ...
    expect(
      state.lanes.lanes.find((l: { id: string }) => l.id === "codex-subscription").capabilities
    ).toEqual({
